 ...
 * repositioned absolutely after every change in the scroll object, and then
...
// A constant fully zoomed out scroll object for the main canvas
const NO_SCROLL = {
  translateX: 0,
  translateY: 0,
  zoom: 0,
 ...
 * @param  {Object} scroll
 */
function drawText (ctx, node, borderWidth, ratio, scroll) {
 ...
 * @param  {Object} scroll
 */
function drawBox (ctx, node, borderWidth, ratio, scroll) {
 ...
 * @param  {CanvasRenderingContext2D} ctx
 * @param  {Array} nodes
 * @param  {Objbect} scroll
 */
function drawTreemap (window, {canvas, ctx}, nodes, scroll) {
  let ratio = window.devicePixelRatio;
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    drawBox(ctx, node, borderWidth, ratio, scroll);
    drawText(ctx, node, borderWidth, ratio, scroll);
  }
}
 ...
 *         and contexts
 * @param  {Object} scroll
 *         The current state of the scroll
 */
module.exports = function setupDraw (window, d3, census, canvases, scroll) {
  ...
  function drawFullTreemap() {
    ...
    drawTreemap(window, canvases.zoom, nodes, scroll);
  };
